...
async function onAnimalRegistration(animalRegistration) {
    console.log('onAnimalRegistration');
    if (animalRegistration.animal.movementStatus !== 'IN_FIELD') {
...
/**
 *
 * @param {org.acme.Z2BTestNetwork..AnimalMovementDeparture} movementDeparture - model instance
 * @transaction
 */
...
async function onAnimalMovementDeparture(movementDeparture) {  // eslint-disable-line no-unused-vars
    ...
    movementDeparture.animal.movementStatus = 'IN_TRANSIT';
     ...
     // save the animal
    const ar = await getAssetRegistry('org.acme.Z2BTestNetwork..Animal');
    ...
    // save the business
    const br = await getAssetRegistry('org.acme.Z2BTestNetwork..Business');
    await br.update(movementDeparture.to);
}

/**
 *
 * @param {org.acme.Z2BTestNetwork..AnimalRegistrationToFederal} registration - model instance
 * @transaction
 */
  
// check registration of a cow
async function checkRegistration(registeredCow) {  // eslint-disable-line no-unused-vars
    console.log('checkRegistration');
    if (registration.animal.movementStatus !== 'IN_REGISTRATION') {
        throw new Error('Animal is already IN_REGISTRATION');
    }

     // set the registration status of the animal
    registration.animal.movementStatus = 'IN_REGISTRATION';

     // register the animal
    const ra = await getAssetRegistry('org.acme.Z2BTestNetwork..Animal');
    await ra.update(registration.animal);

    // add the animal to the incoming animals of the
    // destination business
    if (registration.to.incomingAnimals) {
        registration.to.incomingAnimals.push(movementDeparture.animal);
    } else {
        movementDeparture.to.incomingAnimals = [movementDeparture.animal];
    }
...
/**
 *
 * @param {org.acme.Z2BTestNetwork..AnimalMovementArrival} movementArrival - model instance
 * @transaction
 */
...
/**
 *
 * @param {org.acme.Z2BTestNetwork..SetupDemo} setupDemo - SetupDemo instance
 * @transaction
 */
